...
module.exports = function (session)
{
    ...
    CassandraStore.prototype.set = function (sid, sess, callback)
    {
        ...
        var query = "UPDATE " + this.table + " SET session=?, expires=? WHERE sid=?;";
        var qParams = [session, expires, sid];
        debug("Query: %s", query);
        this.client.execute(query, qParams, this.clientOptions.queryOptions,
        function (error, result)
        {
    ...
    {
        var query = "SELECT session FROM " + this.table + " ALLOW FILTERING;";
        var qParams = [];
        debug("Query: %s", query);
        debug("Query params: %s", qParams);
        this.client.execute(query, qParams, this.clientOptions.queryOptions,
    ...
    {
        var query = "SELECT count(*) FROM " + this.table + " ALLOW FILTERING;";
        var qParams = [];
        debug("Query: %s", query);
        debug("Query params: %s", qParams);
        this.client.execute(query, qParams, this.clientOptions.queryOptions,
        function (error, result)
        {
            ...
            else
            {
                if (result && result.rows)
                {
                    ...
                    if (result.rows.length > 0)
                    {
                        length = result.rows[0].get("count").toNumber();
                    }
                }
                debug("Length of active sessions: %s", length);
            }
