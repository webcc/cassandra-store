...
module.exports = class CassandraStore extends Store
{
    ...
    length(callback)
    {
        ...
        this.client.execute(query, qParams, this.clientOptions.queryOptions, (error, result) =>
        {
            ...
            else
            {
                ...
                debug(`Length of active sessions: ${length}.`);
            }
    ...
    set(sid, sess, callback)
    {
        ...
        const expires = new Date(Date.now() + sess.cookie.maxAge);
        const query = `INSERT INTO ${this.table} (sid, session, expires) VALUES (?, ?, ?);`;
    ...
    touch(sid, session, callback)
    {
        this.get(sid, (error, currentSession) =>
        {
            if (error)
            {
                ...
                return callback(error, currentSession);
            }
            else
            {
                // update expiration
                ...
                this.set(sid, currentSession, (err, res) =>
                {
                    ...
                    return callback(err, currentSession);
                });
            }
        });
    };
