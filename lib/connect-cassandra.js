...
/*
    ...
    cassandraStore.prototype.set = function(sid, sess, fn){
        ...
        try {
            ...
            //ttl = this.dbConfig.ttl;

            //ttl = ttl || ('number' === typeof maxAge ? maxAge / 1000 || 0 : this.dbConfig.ttl);
    ...
    cassandraStore.prototype.destroy = function(sid, fn){
        var query = "DELETE from " + tableName + " WHERE sid=?";
        var params = [sid];
        this.client.execute(query, params, queryOptions, function(err) {
            if (err){
                return fn(err);
            }
   ...
   cassandraStore.prototype.delete = function (sid, fn){
